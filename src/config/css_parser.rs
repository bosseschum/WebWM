use cssparser::{Parser, ParserInput, Token, Color as CssColor};
...
fn parse_css_property(line: &str, variables: &HashMap<String, String>) -> Option<(String, StyleValue)> {
    if !line.contains(":") {
        return None;
    }
    ...
    if value_str.starts_with("var(") && value_str.ends_with(")") {
        let var_name = value_str.trim_start_matches("var(")
            .trim_end_matches(")")
            .trim();
...
fn parse_rgb_color(rgb: &str) -> Option<Color> {
    ...
    let content = rgb.trim_start_matches("rgb(")
...
fn parse_named_color(name: &str) -> Option<Color> {
    ...
    match name.to_lowercase().as_str() {
        ...
        "green" => Some(Color { r: 0, g: 255, b: 0, a: 1.0 }),
