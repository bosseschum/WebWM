...
fn parse_bar(node: &Node) -> Option<BarConfig> {
    ...
    let height = node
        ...
        .unwrap_or(30);
    let class = node.attribute("class").unwrap_or("bar").to_string();
...
fn parse_widget(node: &Node) -> Option<Widget> {
    match node.tag_name().name() {
        "workspaces" => Some(Widget::Workspaces {
            display: node.attribute("display").unwrap_or("icons").to_string(),
        }),
        "window-title" => Some(Widget::WindowTitle {
            max_width: node.attribute("max-width").and_then(|w| w.parse().ok()),
        }),
        "system-tray" => Some(Widget::SystemTray),
        "clock" => Some(Widget::Clock {
            format: node.attribute("format").unwrap_or("%H:%M").to_string(),
        }),
    ...
    node.children()
        ...
        .filter_map(|ws| {
            ...
            let name = ws
                ...
                .unwrap_or(&format!("{}", id))
                .to_string();
            let layout = ws.attribute("layout").unwrap_or("tiling").to_string();
...
fn parse_layout(node: &Node) -> LayoutSettings {
    ...
    for child in node.children().filter(|n| n.is_element()) {
        match child.tag_name().name() {
            "tiling" => {
                if let Some(gaps) = child.children().find(|n| n.tag_name().name() == "gaps") {
                    settings.gaps_outer = gaps
                        .attribute("outer")
                        .and_then(|g| g.parse().ok())
                        .unwrap_or(10);
                    settings.gaps_inner = gaps
                        .attribute("inner")
                        .and_then(|g| g.parse().ok())
                        .unwrap_or(10);
                }

                if let Some(ratio) = child
                    .children()
            ...
            "floating" => {
                if let Some(size) = child
                    .children()
                    .find(|n| n.tag_name().name() == "default-size")
                {
                    settings.floating_default_width = size
                        .attribute("width")
                        .and_then(|w| w.parse().ok())
                        .unwrap_or(800);
                    settings.floating_default_height = size
                        .attribute("height")
                        .and_then(|h| h.parse().ok())
                        .unwrap_or(600);
                }

                if let Some(center) = child
                    .children()
